...
import { ROOT_ROUTE_NAME, SKYROC_ROUTER_TYPES_MODULE_NAME } from '../constants';
 ...
 * 生成路由文件
 *
 * @param nodes - The nodes to generate routes for
 * @param statInfo - The stat info to generate routes for
 * @param options - The options to generate routes for
 */
...
async function updateRoutes(
  ...
  if (updatedNames.length > 0) {
    ...
    updatedRoutes.forEach(node => {
      ...
      // 更新布局
      updateStringProperty(routeElement, 'layout', 'base');
    });
  }
...
import type { AutoRouterRoute } from '${SKYROC_ROUTER_TYPES_MODULE_NAME}';
